#!/bin/zsh
# clone_independent.sh — Clone a VM instance with a new identity and a distinct SSH port.
#
...
#
# The chosen SSH port is saved to <dst>/ports.env so cfw_install and iproxy
# commands pick up the right port for this instance automatically.
#
# Usage:
#   zsh scripts/clone_independent.sh <source> <destination> [--ssh-port PORT]
#
# Examples:
#   zsh scripts/clone_independent.sh iphone_01 iphone_02
#   zsh scripts/clone_independent.sh iphone_01 iphone_02 --ssh-port 2224
#   zsh scripts/clone_independent.sh vm iphone_backup --ssh-port 2225
...
usage() {
  cat <<'EOF'
Usage: clone_independent.sh <source> <destination> [--ssh-port PORT]

  <source>       Existing VM directory (e.g. iphone_01 or vm)
  <destination>  New VM directory to create (e.g. iphone_02)

Options:
  --ssh-port PORT   Local iproxy port mapped to guest SSH port 22 (default: 2223)
...
Notes:
  ...
  - SSH_PORT is saved to <destination>/ports.env (sourced by cfw_install scripts).
...
iproxy setup (after first boot):
  source <destination>/ports.env
  iproxy -u <NEW_UDID> $SSH_PORT 22 &
...
[[ "$SSH_PORT" == <1-65535> ]] || die "Invalid --ssh-port: ${SSH_PORT}"
...
# ─── Write ports.env ─────────────────────────────────────────────
info "Writing port assignments..."

cat > "${DST}/ports.env" <<EOF
# Port assignments for ${DST_NAME}
# Usage: source ${DST_NAME}/ports.env
# Generated by clone_independent.sh on $(date -u '+%Y-%m-%dT%H:%M:%SZ')
SSH_PORT=${SSH_PORT}
EOF

ok "  ports.env written  (SSH_PORT=${SSH_PORT})"
...
echo ""
echo "  After first boot, find the new UDID in the VM log, then set up SSH:"
echo "    source ${DST_NAME}/ports.env"
echo "    iproxy -u <NEW_UDID> \$SSH_PORT 22 &"
echo "    ssh -p \$SSH_PORT root@localhost"
echo ""
echo "  CFW install (SSH_PORT sourced from ports.env):"
echo "    source ${DST_NAME}/ports.env && make cfw_install VM_DIR=${DST_NAME}"
echo ""
